...
\emph{Case P2(n+1):}\\
\begin{align*}
fib(3*(n+1)+1) &= fib(3n+4) &\pnote{Arithmetic}\\
&=fib(3n+3) + fib(3n+2) &\pnote{Definition of fib(3n+4)}\\
&=even_odd &\pnote{Induction hypothesis P3(n), P1(n+1)}\\
&=odd &\pnote{Arithmetic}\\
\end{align*}
Therefore, \emph{P2}(n) holds for all $n \in \mathbb{N}$ by weak induction.
\bigbreak


\emph{Case P3(n+1):}\\
\begin{flalign*}
fib(3*(n+1)+2) &= fib(3n+5) &\pnote{Arithmetic}\\
&=fib(3n+4) + fib(3n+3) &\pnote{Arithmetic}\\
&= even + odd &\pnote{Induction hypothesis P1(n+1), P2(n+1)}\\
&= odd &\pnote{Arithmetic}\\
\end{flalign*}
Therefore, \emph{P3}(n) holds for all $n \in \mathbb{N}$ by weak induction.
\end{proof}
